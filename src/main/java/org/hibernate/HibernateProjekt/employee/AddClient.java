...
public class AddClient extends GridPane {
	...
	public AddClient(SessionFactory factory) {
	...
	public static void addClient(TextField[] fields, SessionFactory factory) {
		Session session = factory.getCurrentSession();
		int max = 0;
		
		Klient cli = new Klient();
		Adres ad = new Adres();
		...
		try {
	        ...
	        sql = "Select p from " + Klient.class.getName() + " p "
	        		+ "order by p.id ";
	        ...
	        cli.setID(max+1);
	        cli.setImie(fName);
	        cli.setNazwisko(lName);
	        cli.setIdAdres(ad.getID());
	        cli.setIdUzytkownika(usr.getID());
	        
	        System.out.println(usr.getID() + usr.getLogin() + usr.getHaslo() + usr.getRola() + ad.getID() + ad.getMiasto() + ad.getKodPocztowy()
	        			+ad.getUlica() + ad.getNrBudynku() + ad.getNrMieszkania() + cli.getID() + cli.getImie() + cli.getNazwisko() + cli.getIdAdres()
	        			+cli.getIdUzytkownika());
